...
function population()
  ...
  local pilottoadd = 100
	local officertoadd = 5
      ...
      if Player_HasShipWithBuildQueue(playerIndex) == 1 then  
        playerIndexList = playerIndex + 1
				if hwsaving[3] == 0 or year == 0 then					 		
					honortimefactor = (1-(year/300)) -- con set a 250: 0.9 a 25 anni, 0.8 a 50 anni, 0.6 a 100 anni - >n >honortimefactor
					if honortimefactor < 0.35 then
						honortimefactor = 0.35
					end        					
					honoramountfactor = (1-(honorList[playerIndexList]/800))  -- a con set a 730: 365 è 0.5   
          ...
          honorList[playerIndexList] = 0					
					...
					--player  
						...
						honorbattle = floor (((Stats_TotalKillsInRUs( playerIndex ) * 1.045) - (Stats_TotalLossesInRUs( playerIndex ) * 0.725) + (Stats_Extra[playerIndexList] * 0.775)) / 1000)        
						honorcapture = floor (((Stats_EnemyShipsCaptured( playerIndex ) * 3500) - (Stats_OwnShipsCaptured( playerIndex ) * 3500)) / 1000)
						honortribute = floor (((Stats_TributeDonated( playerIndex ) * 0.5) - (Stats_TributeReceived( playerIndex ) * 0.25)) / 1000)
