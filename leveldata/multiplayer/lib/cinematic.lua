function Cinematic()    
  if leveltype ~= "benchmark" then     
    ...
    UI_BindKeyEvent(ESCKEY, "tomainmenu")    
		if Player_NumberOfAwakeShips(0)<=1 or Player_NumberOfAwakeShips(1)<=1 then
			Rule_Add("tomainmenu")
			return nil
		end   		   
		if Universe_GameTime() > 25 and Universe_GameTime() < 30 then		
      cinecontatore[3] = 2 --focus		
			UI_BindKeyEvent(F1KEY, "caminc") 	
			UI_BindKeyEvent(F2KEY, "camstop")	
			UI_BindKeyEvent(F3KEY, "camfocus")
			UI_BindKeyEvent(F4KEY, "camzoom")		
			UI_BindKeyEvent(F5KEY, "freecontrols")			
			Subtitle_Message("[F1 Rate], [F2 Manual], [F3 Focus], [F4 Zoom], [F5 Controls], [ESC Exit]", 20)
    end			
	end			
  ...
  if cinecontatore[1] == 0 or cinecontatore[3] > 0 then
		...
		for z = 1,6,1 do
          ...
          if cinecontatore[3] == 0 or (leveltype == "benchmark" and Universe_GameTime() < 25) then	      
						if SobGroup_Selected("splitPlayer_Ships"..(z-1) .. tostring(i)) == 1 or leveltype == "benchmark" then						
      ...
      if cinecontatore[2] > 0 then	
        ...
        UI_UnBindKeyEvent(F5KEY)	        				
		  ...
		  cinecontatore[6] = cinecontatore[2]
			if leveltype ~= "benchmark" then
				Subtitle_Message("[F1 Rate], [F2 Manual], [F3 Focus], [F4 Zoom], [F5 Controls], [ESC Exit]", 10)
			end	
			Event_Start("startcinematic")			
		end				
	end 
	--se non trova navi (es. attackers) resetta a cinematica su selezione, se c'è camera on selection ripristina il contatore
	if cinecontatore[2] == 0 then
    ...
    --trova un punto di focus	
		...
		while escidaciclo <= cinecontatore[2] do	   
			if SobGroup_Empty("selectedCinematic"..gestionefocus..rr) == 0 and SobGroup_AreAnyOfTheseTypes("selectedCinematic"..gestionefocus..rr, "supporter") == 0 then
				...
				SobGroup_Clear("focus") 
				SobGroup_FillSubstract("temp", "Player_Ships"..Universe_CurrentPlayer(), "supporter"..Universe_CurrentPlayer())
    ...
    if SobGroup_AreAnyOfTheseTypes("focus", "hgn_commandfortress, vgr_commandfortress, hgn_ark, hgn_battlestation") == 1 then
      ...
      elseif r == 10 or r == 11 or r == 12 then  				  
			...
			elseif SobGroup_GetActualSpeed("focus") > 169 or 
				 SobGroup_FillProximitySobGroup ("tempcinematic", "productioncapital"..Universe_CurrentPlayer(), "focus", 1200) == 1 then
				if cinecontatore[7] == 1 then
					r = random (1,2)
				elseif cinecontatore[7] == 0 then
					r = random (3,5)
...
function caminc() 
  cinecontatore[4] = cinecontatore[4] + 1
	...
	if cinecontatore[4] == 5 then
	  Subtitle_Message("Rate random", 3) 
	elseif cinecontatore[4] == 4 then	  
	  Subtitle_Message("Rate X5", 3)
	elseif cinecontatore[4] == 3 then	  
	  Subtitle_Message("Rate X4", 3)
	elseif cinecontatore[4] == 2 then	  
	  Subtitle_Message("Rate X3", 3)
	elseif cinecontatore[4] == 1 then	  
	  Subtitle_Message("Rate X2", 3)
  elseif cinecontatore[4] == 0 then	  
	  Subtitle_Message("Rate X1", 3) 		
	end 
	Rule_Remove("caminc")
end
...
function camfocus() 
  cinecontatore[3] = cinecontatore[3] + 1
	...
	if cinecontatore[3] == 0 then
	  Subtitle_Message("Focus selection", 3) 	
	elseif cinecontatore[3] == 1 then	  
	  Subtitle_Message("Focus enemy", 3)	
	elseif cinecontatore[3] == 2 then	  
	  Subtitle_Message("Focus random", 3)	
	end 
...
function camzoom() 
  cinecontatore[7] = cinecontatore[7] + 1
	...
	if cinecontatore[7] == 2 then
	  Subtitle_Message("Zoom random", 3) 	
	elseif cinecontatore[7] == 1 then	  
	  Subtitle_Message("Zoom X2", 3)
  elseif cinecontatore[7] == 0 then	  
	  Subtitle_Message("Zoom X1", 3)		
	end 
	Rule_Remove("camzoom")
end
